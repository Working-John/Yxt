...
function partialRefresh() {
    $("#fileList ul").html(`<li class="fl-li" id="addFile"><img src="../../img/添加.png"><label>添加文件</label></li>`);
    $.ajax({
        type: "get",
        url: '/upfile/list',
        async: true,
...
$(function () {
    ...
    $('body').on("dblclick", ".fl-li", function (e) {
        ...
        window.open(urls.server + '/upfile/download/' + fileId);
    });
    /**
     * 展开/收起进度条
     */
    ...
    let uploader = $.uploaderInit({
        server: urls.server + '/upfile/breakpoint-upload',
        pick: {
            id: '#addFile',
            multiple: true
        },
            ...
            WebUploader.Uploader.register({
                ...
                beforeSendFile: function (file) {
                    ...
                    (new WebUploader.Uploader()).md5File(file, 0, 5242880).progress(function (percentage) {
                        ...
                        // 进行md5判断
                        $.ajax({
                            url: urls.server + '/upfile/check-file',
                            type: 'GET',
                            showError: false,
                            global: false,
                            data: {
