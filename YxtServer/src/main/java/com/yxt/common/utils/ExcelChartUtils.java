...
public class ExcelChartUtils {
     ...
     * @throws IOException
     */
    public static void createBarChart(XSSFSheet sheet, int length) {
         //创建一个sheet页
        ...
        boolean result = drawSheet0Map(sheet, STBarGrouping.CLUSTERED, length);
     ...
     * @return
     */
    private static boolean drawSheet0Map(XSSFSheet sheet, Enum group, int length) {
        ...
        // 画一个图区域
        // 前四个默认0，从第0行到第16行,从第13列到第19列的区域  列 行 列 行
        ClientAnchor anchor = drawing.createAnchor(0, 0, 0, 0, 13, 0, 19, 16);
        // 创建一个chart对象
        Chart chart = drawing.createChart(anchor);
        XSSFChart c =  (XSSFChart) chart;
        c.setTitleText("各部门事项、耗时统计");
        ...
        for (int i = 0; i < 2; i++) {
            ...
            // 选定区域第0行,第10、11列标题作为图例
            String legendDataRange = new CellRangeAddress(0, 0, i + 10, i + 10).formatAsString(sheetName, true);
            ...
            // 选第0列,第1-6行作为横坐标区域
            String axisDataRange = new CellRangeAddress(1, length, 9, 9).formatAsString(sheetName, true);
            ctStrRef.setF(axisDataRange);
            // 数据区域
            CTNumDataSource ctNumDataSource = ctBarSer.addNewVal();
            CTNumRef ctNumRef = ctNumDataSource.addNewNumRef();
            // 选第1-6行,第1-3列作为数据区域 //1 2 3
            String numDataRange = new CellRangeAddress(1, length, i + 10, i + 10).formatAsString(sheetName,
                    true);
     ...
     * @return
     */
    private boolean drawSheet1Map(XSSFSheet sheet, String type, List<String> fldNameArr,
     ...
     * @throws IOException
     */
    public static void createPieChart(XSSFSheet sheet) {
        drawSheet2Map(sheet, "is3D");
    }
    /**
     * 创建饼状图
     *
     ...
     * @return
     */
    private static boolean drawSheet2Map(XSSFSheet sheet, String type) {
        boolean result = false;
        // 获取sheet名称
        String sheetName = sheet.getSheetName();
        //result = drawSheet0Table(sheet, titleArr, fldNameArr, dataList);
        ...
        // 画一个图区域
        // 前四个默认0，从第8行到第25行,从第0列到第6列的区域
        ClientAnchor anchor = drawing.createAnchor(0, 0, 0, 0, 3, 10, 10, 25);
        // 创建一个chart对象
        Chart chart = drawing.createChart(anchor);
        XSSFChart c =  (XSSFChart) chart;
        c.setTitleText("各事项耗时统计");
        ...
        CTStrRef ctStrRef = ctSerTx.addNewStrRef();
         //选定区域第0行,第1,2,3列标题作为图例 //1 2 3
        String legendDataRange = new CellRangeAddress(0, 0, 5, 5).formatAsString(sheetName, true);
        ctStrRef.setF(legendDataRange);
        ...
        ctStrRef = cttAxDataSource.addNewStrRef();
        // 选第0列,第1-6行作为横坐标区域
        String axisDataRange = new CellRangeAddress(1, 8, 4, 4).formatAsString(sheetName, true);
        ctStrRef.setF(axisDataRange);
        // 数据区域
        CTNumDataSource ctNumDataSource = ctPieSer.addNewVal();
        CTNumRef ctNumRef = ctNumDataSource.addNewNumRef();
        // 选第1-8行,第5列作为数据区域 //1 2 3
        String numDataRange = new CellRangeAddress(1, 8, 5, 5).formatAsString(sheetName,
                true);
        ctNumRef.setF(numDataRange);
        // 显示边框线
     ...
     * @return
     */
    private boolean drawSheet3Map(XSSFSheet sheet, String type, List<String> fldNameArr,
                                  List<Map<String, Object>> dataList, List<String> titleArr) {
        boolean result = false;
        // 获取sheet名称
        String sheetName = sheet.getSheetName();
        ...
        for (int i = 0; i < fldNameArr.size() - 1; i++) {
            ...
            ctStrRef = cttAxDataSource.addNewStrRef();
            // 选第0列,第1-6行作为横坐标区域
            String axisDataRange = new CellRangeAddress(1, dataList.size(), 4, 4).formatAsString(sheetName, true);
            ctStrRef.setF(axisDataRange);
            // 数据区域
     ...
     * @return 是否生成成功
     */
    private static boolean drawSheet0Table(XSSFSheet sheet, List<String> titleArr, List<String> fldNameArr,
                                    List<Map<String, Object>> dataList) {
        // 测试时返回值
        boolean result = true;
        // 初始化表格样式
